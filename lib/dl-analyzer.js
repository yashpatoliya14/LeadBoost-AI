...
function analyzeHeadlineEffectiveness(headline) {
  if (!headline || headline.length < 5) return { score: 0, features: {} };
  
  const doc = nlp(headline);
  const tokens = natural.WordTokenizer.tokenize(headline.toLowerCase());
  
  // Feature detection
  const hasNumber = /\d+/.test(headline);
  const hasActionVerb = doc.verbs().out('array').some(verb => 
...
function analyzeCtaEffectiveness(ctaText) {
  if (!ctaText || ctaText.length < 2) return { score: 0, features: {} };
  
  const tokens = natural.WordTokenizer.tokenize(ctaText.toLowerCase());
  const doc = nlp(ctaText);
  
  // CTA best practices
  const startsWithVerb = doc.verbs().out('array').length > 0 && 
...
function analyzeSpecificity(text) {
  ...
  const properNouns = doc.match('#ProperNoun').out('array').length;
...
export async function analyzeMl(extractedContent) {
  try {
    console.log('Starting Deep Learning analysis...');
    
    // Initialize models
    await initializeModels();
    
    // Parallel analysis of all content sections
    const [
      headlineSentiment,
      subheadlineSentiment,
      bodyCopySentiment,
      headlineEffectiveness,
      ctaEffectiveness,
    ] = await Promise.all([
      sentimentAnalyzer(extractedContent.headline),
      sentimentAnalyzer(extractedContent.subheadline),
      sentimentAnalyzer(extractedContent.bodyCopy),
      Promise.resolve(analyzeHeadlineEffectiveness(extractedContent.headline)),
      Promise.resolve(analyzeCtaEffectiveness(extractedContent.cta[0] || '')),
    ]);
    ...
    const mlScores = {
      ...
      subheadline: {
        ...
        sentiment: subheadlineSentiment[0],
      ...
      overall: {
        ...
        confidence: 'High',
        modelVersion: 'DistilBERT + Linguistic Analysis v1.0'
      }
    };
